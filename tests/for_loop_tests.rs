...
fn test_for_loop_countdown() {
    ...
    let mut parser =
        Parser::new("(for (let i 10) (> i 0) (-- i) (let count (+ count 1)))").unwrap();
    axe.eval(parser.parse().unwrap()).unwrap();
...
fn test_for_loop_with_array() {
    ...
    let mut parser =
        Parser::new("(for (let i 0) (< i (len numbers)) (++ i) (let sum (+ sum (get numbers i))))")
            .unwrap();
    axe.eval(parser.parse().unwrap()).unwrap();
...
fn test_nested_for_loops() {
    ...
    let code =
        "(for (let i 1) (<= i 3) (++ i) (for (let j 1) (<= j 3) (++ j) (let product (* i j))))";
...
fn test_for_loop_in_function() {
    ...
    let code =
        "(fn sum_to_n (n) (let sum 0) (for (let i 1) (<= i n) (++ i) (let sum (+ sum i))) sum)";
    let mut parser = Parser::new(code).unwrap();
    axe.eval(parser.parse().unwrap()).unwrap();
