...
fn let_creates_variable_if_not_exists() {
    ...
    let result = axe
        .eval(Expr::Set("x".into(), Box::new(Expr::Int(10))))
        .unwrap();
...
fn assign_using_expression() {
    ...
    // Create variable x = 5
    axe.eval(Expr::Set("x".into(), Box::new(Expr::Int(5))))
        .unwrap();
...
fn let_in_nested_block_updates() {
    ...
    let block = Expr::Block(vec![
        ...
        Expr::Block(vec![Expr::Set("x".into(), Box::new(Expr::Int(20)))]),
...
fn let_in_same_scope_updates() {
    ...
    axe.eval(Expr::Set("x".into(), Box::new(Expr::Int(100))))
        .unwrap();
